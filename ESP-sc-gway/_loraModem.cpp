...
struct espGwayConfig gwayConfig;
...
// Set Bandwidth
static void setBw(uint16_t bw){
	uint8_t mc1 = readRegister(REG_MODEM_CONFIG1);
	mc1 &= ~0xF0;
	if(bw == 125){
		writeRegister(REG_MODEM_CONFIG1, mc1 | 0x70);
	}else if(bw == 500){
		writeRegister(REG_MODEM_CONFIG1, mc1 | 0x90);
	}
}
// ----------------------------------------------------------------------------
...
void hop() {
	...
	global_sf = SF7;													// Starting the new frequency 
	setRate(global_sf, 0x40);											// set the sf to SF7 
...
bool sendPkt(uint8_t *payLoad, uint8_t payLength)
{
	...
	writeRegister(REG_FIFO_ADDR_PTR, 0x00);//(uint8_t) readRegister(REG_FIFO_TX_BASE_AD));	// 0x0D, 0x0E
	
	writeRegister(REG_PAYLOAD_LENGTH, payLength);				// 0x22
	payLoad[payLength] = 0x00;
	writeBuffer(REG_FIFO, payLoad, payLength);
	return true;
}
						...
						uint8_t powe, uint32_t freq, uint8_t crc, uint8_t iiq, uint16_t bw)
{
	...
	setBw(bw); // ESTA llamada despues de setRate
